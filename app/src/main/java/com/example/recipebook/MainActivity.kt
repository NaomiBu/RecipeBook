...
fun Homescreen (onNextScreen: (String) -> Unit, modifier: Modifier = Modifier) {
    ...
    Column () {
        ...
        Button(onClick = { onNextScreen("mexican_burrito") }) {
            Text(text = recipes[0])
        }
        Button(onClick = { onNextScreen("chicken_katsu_curry") }) {
            Text(text = recipes[1])
        }
        Button(onClick = { onNextScreen("waakye") }) {
            Text(text = recipes[2])
        }
        Button(onClick = { onNextScreen("lasagne") }) {
            Text(text = recipes[3])
        }
        Button(onClick = { onNextScreen("lamb_biryani") }) {
            Text(text = recipes[4])
        }
    }
}
